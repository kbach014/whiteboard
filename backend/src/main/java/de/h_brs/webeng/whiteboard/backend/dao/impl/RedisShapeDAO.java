...
public class RedisShapeDAO implements ShapeDAO {
	...
	@Override
	public HashMap<String, List<Shape>> findAllShapesFromWB(Whiteboard whiteboard) throws WhiteboardNotFoundException {
		try (Jedis jedis = MyJedisPool.getPool("localhost").getResource()) {
			...
			for (int x = 0; x < registeredUseres.size(); ++x) {
				String shapeKeys = "whiteboard:" + wbID + ":user:" + registeredUseres.get(x).getUsername() + ":shapes";
